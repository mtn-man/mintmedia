...
func main() {
	...
	configPath := pflag.String(
		"config",
		"",
		"Path to config.toml (default: ~/.config/mintmedia/config.toml; honors $XDG_CONFIG_HOME)",
	)
	...
	pflag.Usage = func() {
		...
		writeln("\nOther flags:")
		writeln("  --config <path>      Path to config.toml (default: ~/.config/mintmedia/config.toml; honors $XDG_CONFIG_HOME)")
...
func newGoProcessor(res *config.Resolved, logger logging.Logger) (processor.Processor, error) {
	pcfg := processor.Config{
		...
		PreserveEpisodeTitles:    res.PreserveEpisodeTitles,
	}
